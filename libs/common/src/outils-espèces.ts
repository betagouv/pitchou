...
import type {
  ...
  FloreAtteinteOds_V1,
  OiseauAtteintOds_V1,
} from "@pitchou/types/espècesFichierOds.d.ts";
...
async function importDescriptionMenacesEspècesFromOdsArrayBuffer_version_1(
  ...
  const odsContent = tableRawContentToObjects(rawContent) as FichierEspècesImpactéesOds_V1;
  ...
  lignesOiseauOds =
    lignesOiseauOds && lignesOiseauOds.map(normaliserCD_REF).filter(ligneEspèceImpactéeHasCD_REF);
  lignesFauneNonOiseauOds =
    lignesFauneNonOiseauOds &&
    lignesFauneNonOiseauOds.map(normaliserCD_REF).filter(ligneEspèceImpactéeHasCD_REF);
  lignesFloreOds =
    lignesFloreOds && lignesFloreOds.map(normaliserCD_REF).filter(ligneEspèceImpactéeHasCD_REF);
