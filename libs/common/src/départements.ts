/**
 * Liste officielle des départements français (métropole, outre-mer et codes particuliers),
 * identifiés par leur code INSEE et leur nom.
 *
 * Les dossiers stockent le code du département (ex : "88", "2A", "971", "99" pour l'étranger).
 */
export type Département = { code: string; nom: string };

export const départements: Département[] = [
  { code: "01", nom: "Ain" },
  { code: "02", nom: "Aisne" },
  { code: "03", nom: "Allier" },
...
/** Nom du département par code INSEE */
export const nomParCodeDépartement: Map<string, string> = new Map(
  départements.map(({ code, nom }) => [code, nom]),
);
