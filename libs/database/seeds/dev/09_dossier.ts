...
export async function seed(knex: Knex) {
  await knex.transaction(async (transaction) => {
    ...
    // Build a seedId → actual DB id map for cross-referencing related entities.
    ...
    for (const { id: seedId, groupe_instructeur, ...dossierData } of SEED_DOSSIERS) {
    ...
    for (const { dossier: seedDossierId, ...evtData } of SEED_ÉVÈNEMENTS_PHASE_DOSSIER) {
      const dossierId = dossierIdMap[seedDossierId];
      if (!dossierId) {
        console.warn(
          `  ⚠ évènement phase "${evtData.phase}" — seed dossier id ${seedDossierId} non résolu`,
        );
        continue;
      }
    ...
    // Step 3 — avis experts
    for (const { dossier: seedDossierId, ...avisData } of SEED_AVIS_EXPERTS) {
      const dossierId = dossierIdMap[seedDossierId];
      if (!dossierId) {
        console.warn(
          `  ⚠ avis_expert ${avisData.id} — seed dossier id ${seedDossierId} non résolu`,
        );
        continue;
      }
    ...
    for (const { dossier: seedDossierId, ...daData } of SEED_DÉCISIONS_ADMINISTRATIVES) {
      const dossierId = dossierIdMap[seedDossierId];
      if (!dossierId) {
        console.warn(
          `  ⚠ décision_administrative ${daData.id} — seed dossier id ${seedDossierId} non résolu`,
        );
        continue;
      }
