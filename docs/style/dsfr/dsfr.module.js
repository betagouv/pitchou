...
class Registration {
  constructor(selector, InstanceClass, creator) {
    ...
    this._property =
      this._instanceClassName.substring(0, 1).toLowerCase() + this._instanceClassName.substring(1);
...
class Element$1 {
  ...
  getDescendantInstances(instanceClassName, stopAtInstanceClassName, stopAtFirstInstance) {
    ...
    for (const child of this._children) {
      ...
      if (
        (!stopAtInstanceClassName || !child.hasInstance(stopAtInstanceClassName)) &&
...
const getKeyCode = (keyCode) =>
  Object.values(KeyCodes).filter((entry) => entry.value === keyCode)[0];
    ...
    mutations.forEach((mutation) => {
      switch (mutation.type) {
        ...
        case "attributes":
          ...
          if (this.modifications.indexOf(mutation.target) === -1)
...
class Instance {
  ...
  unlisten(type, closure, options) {
    ...
    const removal = listeners.filter(
      (listener) => listener.closure === closure && listener.matchOptions(options),
    );
  ...
  unlistenKey(code, closure) {
    ...
    this._keyListenerTypes.forEach((type) => {
      if (!this._keys.some((key) => key.type === type))
...
class Listener {
  ...
  matchOptions(options = null) {
    switch (true) {
      case options === null:
      case typeof this._options === "boolean" &&
...
class Collapse extends Disclosure {
  ...
  _electPrimaries(candidates) {
    const primary = this.element.parent.instances
      .map((instance) => instance.collapsePrimary)
      .filter((button) => button !== undefined && candidates.indexOf(button) > -1);
...
class Placement extends Instance {
  constructor(
    ...
    aligns = [PlacementAlign.CENTER, PlacementAlign.START, PlacementAlign.END],
    safeAreaMargin = 16,
  ) {
    super();
    this._mode = mode;
  ...
  getHorizontalAlign() {
    for (const align of this._aligns) {
      switch (align) {
        case PlacementAlign.CENTER:
          if (
            this.referentRect.center - this.rect.width * 0.5 > this.safeArea.left &&
            this.referentRect.center + this.rect.width * 0.5 < this.safeArea.right
          )
            return PlacementAlign.CENTER;
          break;

        case PlacementAlign.START:
          if (this.referentRect.left + this.rect.width < this.safeArea.right)
            return PlacementAlign.START;
          break;

        case PlacementAlign.END:
          if (this.referentRect.right - this.rect.width > this.safeArea.left)
            return PlacementAlign.END;
          break;
      }
    }
  ...
  getVerticalAlign() {
    for (const align of this._aligns) {
      switch (align) {
        case PlacementAlign.CENTER:
          if (
            this.referentRect.middle - this.rect.height * 0.5 > this.safeArea.top &&
            this.referentRect.middle + this.rect.height * 0.5 < this.safeArea.bottom
          )
            return PlacementAlign.CENTER;
          break;
...
class Accordion extends api.core.Instance {
  ...
  get collapsePrimary() {
    const buttons = this.element.children
      .map((child) => child.getInstance("CollapseButton"))
      .filter((button) => button !== null && button.hasClass(AccordionSelector.BUTTON));
    return buttons[0];
  }
}
...
class AccordionsGroup extends api.core.CollapsesGroup {
  ...
  validate(member) {
    const match = member.node.matches(
      api.internals.legacy.isLegacy
        ? AccordionSelector.COLLAPSE_LEGACY
...
class Segmented extends api.core.Instance {
  ...
  init() {
    ...
    this._isLegendInline =
      this.legend && this.legend.classList.contains(`${api.prefix}-segmented__legend--inline`);
    this.isResizing = true;
  }
  ...
  resize() {
    ...
    if (this._isLegendInline) {
      ...
      if (
        ...
        this.elements.scrollWidth + this.legend.offsetWidth + gapOffset >
...
class Breadcrumb extends api.core.Instance {
  ...
  get collapsePrimary() {
    const buttons = this.element.children
      .map((child) => child.getInstance("CollapseButton"))
      .filter((button) => button !== null && button.hasClass(BreadcrumbSelector.BUTTON));
    return buttons[0];
  }
}
...
class Tooltip extends api.core.Placement {
  constructor() {
    super(
      api.core.PlacementMode.AUTO,
      [api.core.PlacementPosition.TOP, api.core.PlacementPosition.BOTTOM],
      [api.core.PlacementAlign.CENTER, api.core.PlacementAlign.START, api.core.PlacementAlign.END],
    );
    this.modifier = "";
...
class SidemenuItem extends api.core.Instance {
  ...
  get collapsePrimary() {
    const buttons = this.element.children
      .map((child) => child.getInstance("CollapseButton"))
      .filter((button) => button !== null && button.hasClass(SidemenuSelector.BUTTON));
    return buttons[0];
  }
}
...
class FocusTrap {
  ...
  trapping() {
    ...
    if (focusables.length && focusables.indexOf(document.activeElement) === -1)
      focusables[0].focus();
  ...
  get collapsePrimary() {
    const buttons = this.element.children
      ...
      .filter(
        (button) =>
          button !== null &&
          (button.hasClass(NavigationSelector.BUTTON) ||
            button.hasClass(NavigationSelector.TRANSLATE_BUTTON)),
      );
    return buttons[0];
  }
}
...
class Navigation extends api.core.CollapsesGroup {
  ...
  validate(member) {
    return (
      super.validate(member) &&
      member.element.node.matches(
        api.internals.legacy.isLegacy
          ? NavigationSelector.COLLAPSE_LEGACY
  ...
  handleClick(e) {
    if (
      ...
      !e.target.matches(api.core.DisclosureSelector.PREVENT_CONCEAL)
    ) {
      this.index = -1;
    }
  }
...
class TabPanel extends api.core.Disclosure {
  ...
  _electPrimaries(candidates) {
    ...
    return super
      ._electPrimaries(candidates)
      .filter((candidate) => this.group.list.node.contains(candidate.node));
  }
}
...
class TabsGroup extends api.core.DisclosuresGroup {
  ...
  apply() {
    ...
    for (let i = this._index + 1; i < this.length; i++)
      this.members[i].translate(TabPanelDirection.END);
...
class TabsList extends api.core.Instance {
  ...
  init() {
    ...
    this.listenKey(
      ...
      this.ascend.bind(this, TabEmission.PRESS_KEY, TabKeys.RIGHT),
      true,
      true,
    );
    this.listenKey(
      api.core.KeyCodes.LEFT,
      this.ascend.bind(this, TabEmission.PRESS_KEY, TabKeys.LEFT),
      true,
      true,
    );
    this.listenKey(
      api.core.KeyCodes.HOME,
      this.ascend.bind(this, TabEmission.PRESS_KEY, TabKeys.HOME),
      true,
      true,
    );
    this.listenKey(
      api.core.KeyCodes.END,
      this.ascend.bind(this, TabEmission.PRESS_KEY, TabKeys.END),
      true,
      true,
    );
    this.isResizing = true;
...
class Transcription extends api.core.Instance {
  ...
  get collapsePrimary() {
    const buttons = this.element.children
      .map((child) => child.getInstance("CollapseButton"))
      .filter((button) => button !== null && button.hasClass(TranscriptionSelector.BUTTON));
    return buttons[0];
  }
}
...
api.internals.register(
  api.transcription.TranscriptionSelector.TRANSCRIPTION,
...
const RangeSelector = {
  ...
  RANGE_DOUBLE_STEP:
    api.internals.ns.selector("range--double") + api.internals.ns.selector("range--step"),
...
class Range extends api.core.Instance {
  ...
  init() {
    ...
    if (this.getAttribute(RangeSelector.RANGE_PREFIX))
      this.setPrefix(this.getAttribute(RangeSelector.RANGE_PREFIX));
    if (this.getAttribute(RangeSelector.RANGE_SUFFIX))
      this.setSuffix(this.getAttribute(RangeSelector.RANGE_SUFFIX));
    this.update();
  }
      ...
      if (this.isLegacy) {
        if (progress.left) {
          ...
          this.style.setProperty(
            "background-size",
            `${parseFloat(progress.right) - parseFloat(progress.left)}px ${this._model.isSm ? "8px" : "12px"}`,
          );
        }
      }
    } else {
...
class RangeInput extends api.core.Instance {
  ...
  init() {
    ...
    if (this.isLegacy)
      this.addDescent(RangeEmission.ENABLE_POINTER, this._enablePointer.bind(this));
    this.change();
  }
  ...
  mutate(attributesNames) {
    if (attributesNames.includes("disabled"))
      this.ascend(RangeEmission.DISABLED, this.node.disabled);
    if (
      attributesNames.includes("min") ||
      attributesNames.includes("max") ||
      attributesNames.includes("step")
    ) {
      this.ascend(RangeEmission.CONSTRAINTS, new RangeConstraints(this.node));
      this.change();
    }
...
class HeaderLinks extends api.core.Instance {
  ...
  init() {
    ...
    toolsHtmlIdList = toolsHtmlIdList.map((element) =>
      element.replace('id=\"', "").replace('\"', ""),
    );
      ...
      for (const element of toolsHtmlAriaControlList) {
        ...
        if (toolsHtmlIdList.includes(ariaControlsValue)) {
          toolsHtmlDuplicateId = toolsHtmlDuplicateId.replace(
            ...
            `aria-controls="${ariaControlsValue + copySuffix}"`,
          );
        }
      }
    }
