...
import { byteFormat } from "../scripts/commun/typeFormat.js";
...
async function main() {
    ...
    if (args['lister-dossiers']) {
        ...
        const dossierId = parseInt(args.dossier)
        // @ts-ignore
        const dossierPourGeoMCE = await récupérerDossiersParIds(dossierId)

        if(!dossierPourGeoMCE){
            throw new Error(`Pas de dossier trouvé pour l'id ${dossierId}`)
        }

        const messagePourGeoMCE = await genererMessagesGeoMCE(dossierPourGeoMCE[0])
        console.log(JSON.stringify(messagePourGeoMCE , null, 4))
