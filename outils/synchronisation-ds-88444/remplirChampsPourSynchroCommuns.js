...
/**
 * Remplit les champs communs pour la synchronisation des dossiers DS88444
 * @param {DossierDS88444[]} dossiersDS - Les dossiers DS à synchroniser
 ...
 * @returns {Omit<DossierPourSynchronisation, "demandeur_personne_physique" | "espèces_protégées_concernées" | "espèces_impactées">[]}
 */
export function remplirChampsPourSynchroCommuns(
    dossiersDS,
    pitchouKeyToChampDS,
    pitchouKeyToAnnotationDS,
    donnéesDécisionAdministrativeParNuméroDossier
) {
    const clonedDossiersDS = structuredClone(dossiersDS)
    return clonedDossiersDS.map((
        {
