...
import {
  toDossierTableauSuiviNouvelleAquitaine2023,
...
import { listAllDossiersComplets } from "../scripts/server/database/dossier.ts";
 ...
 */
function trouverDossierEnBDDCorrespondant(
  dossiersStockésEnBaseDeDonnées,
  dossierPartiel88444DepuisTableauSuivi,
) {
  const nomProjetDossierCandidat = dossierPartiel88444DepuisTableauSuivi["Nom du projet"];
  const dossiersAvecCeNom = dossiersStockésEnBaseDeDonnées.filter(
    (d) => d.nom === nomProjetDossierCandidat,
  );
  if (dossiersAvecCeNom.length === 0) return undefined;

  if (dossiersAvecCeNom.length === 1) return dossiersAvecCeNom[0];

  // dossiersAvecCeNom.length >= 2

  const communesDossierTableauSuivi =
    dossierPartiel88444DepuisTableauSuivi["Commune(s) où se situe le projet"];

  if (Array.isArray(communesDossierTableauSuivi)) {
    const dossierAvecNomEt1CommuneEnCommun = dossiersAvecCeNom.find((dossierBDD) => {
      return communesDossierTableauSuivi.some((communeTableauSuivi) =>
        (dossierBDD["communes"] || [])
          //@ts-ignore
          .some(
            (communeDossierBDD) =>
              communeTableauSuivi && communeTableauSuivi.code === communeDossierBDD.code,
          ),
      );
    });
  ...
  const départementsDossierTableauSuivi =
    dossierPartiel88444DepuisTableauSuivi["Département(s) où se situe le projet"];
  ...
  if (Array.isArray(départementsDossierTableauSuivi)) {
    const dossierAvecNomEt1DépartementEnCommun = dossiersAvecCeNom.find((dossierBDD) => {
      return départementsDossierTableauSuivi.some((départementTableauSuivi) =>
        (dossierBDD["départements"] || []).some(
          (départementDossierBDD) =>
            départementTableauSuivi && départementTableauSuivi.code === départementDossierBDD,
        ),
      );
    });
...
const dossiersTableauSuivi = dossiersObject
  .map((dossier) =>
    toDossierTableauSuiviNouvelleAquitaine2023(dossier, nomToCommune, stringToDépartement),
  )
  .filter(estImportable);
const dossiersTableauSuiviAvecNuméroOnagre = dossiersTableauSuivi.filter(
  (d) => d["N°ONAGRE de demande"],
);

const dossierSuiviNAToDossierEtAnnotation88444 = new Map(
  dossiersTableauSuiviAvecNuméroOnagre.map((d) => [
    d,
    {
...
for (const [
  dossierTableauSuivi,
  { dossier: dossierPartiel88444, annotations: annotationsPartielle88444 },
] of dossierSuiviNAToDossierEtAnnotation88444) {
  const dossierEnBaseDeDonnéeCorrespondant = trouverDossierEnBDDCorrespondant(
    dossiersStockésEnBaseDeDonnées,
    dossierPartiel88444,
  );
  ...
  if (!dossierEnBaseDeDonnéeCorrespondant) {
    ...
    if (dossierEnBaseDeDonnéeCorrespondant["historique_identifiant_demande_onagre"]) {
      ...
      if (
        annotationsPartielle88444["N° Demande ONAGRE"] ===
        dossierEnBaseDeDonnéeCorrespondant["historique_identifiant_demande_onagre"]
      ) {
        dossiersTableauDeSuiviDontDossierBDDAvecNuméroOnagreCorrect.add(dossierTableauSuivi);
      } else {
        dossiersTableauDeSuiviVersDossierBDDAvecNuméroOnagreDifférent.set(
          dossierTableauSuivi,
          dossierEnBaseDeDonnéeCorrespondant,
        );
      }
    } else {
      dossiersTableauDeSuiviVersDossierBDDSansNuméroOnagre.set(
        dossierEnBaseDeDonnéeCorrespondant,
        annotationsPartielle88444["N° Demande ONAGRE"],
      );
    }
...
console.log(
  "dossiersTableauDeSuiviVersDossierBDD SansNuméroOnagre",
  dossiersTableauDeSuiviVersDossierBDDSansNuméroOnagre.size,
);
console.log(
  "dossiersTableauDeSuiviVersDossierBDD AvecNuméroOnagreDifférent",
  dossiersTableauDeSuiviVersDossierBDDAvecNuméroOnagreDifférent.size,
);
console.log(
  "dossiersTableauDeSuiviDontDossierBDDAvecNuméroOnagreCorrect",
  dossiersTableauDeSuiviDontDossierBDDAvecNuméroOnagreCorrect.size,
);
console.log(
  "dossiersTableauDeSuiviAvecNuméroOnagreSansDossierBDDCorrespondant",
  dossiersTableauDeSuiviAvecNuméroOnagreSansDossierBDDCorrespondant.size,
);
...
dossiersTableauDeSuiviVersDossierBDDSansNuméroOnagre.forEach((numéroOnagre, dossierBDD) => {
  ...
  annotationsEnAttente.push(
    remplirAnnotations(DEMARCHE_SIMPLIFIEE_API_TOKEN, {
      dossierId,
  ...
  console.log(
    "Annotation onagre remplie",
    numéroOnagre,
    numDS,
    `https://www.demarches-simplifiees.fr/procedures/88444/dossiers/${numDS}/annotations-privees`,
  );
});
