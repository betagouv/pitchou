...
export async function up(knex) {
  await knex.schema.createTable("avis_expert", function (table) {
    ...
    table
      .date("date_saisine")
      .comment("Date à laquelle l'expert a été officiellement saisi pour avis.");

    table.uuid("saisine_fichier").comment("Fichier transmis lors de la saisine de l'expert.");
    table.foreign("saisine_fichier").references("id").inTable("fichier");
    ...
    table.foreign("avis_fichier").references("id").inTable("fichier");
  });
}
