...
<ul class="mx-0 mt-2 mb-0 flex list-none flex-col gap-2 p-0">
  {#each rows as dossier (dossier.id)}
    ...
    >
      <a href="/dossiers/{dossier.id}" class="fr-raw-link block p-4 pr-12 no-underline">
        <div class="flex items-center gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="fr-mb-0 mt-1 truncate text-sm text-gray-600">{formatDemandeur(dossier)}</p>
      ...
      <span
        class="fr-icon-arrow-right-s-line absolute top-1/2 right-3 -translate-y-1/2 text-gray-300 transition group-hover:text-blue-500"
        aria-hidden="true"
      ></span>
    </li>
  {/each}
