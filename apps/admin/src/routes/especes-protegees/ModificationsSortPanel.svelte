<script lang="ts">
  ...
  type Props = {
    selectedSort: ModificationSortKey;
    sortOrder: SortOrder;
    onChange: (sort: ModificationSortKey, order: SortOrder) => void;
  };

  let { selectedSort, sortOrder, onChange }: Props = $props();

  // Select the option, or flip the direction if it is already the active one.
  function selectOrToggleSort(key: ModificationSortKey) {
    if (selectedSort === key) {
...
>
  <legend class="text-[1.25rem] fr-text--bold fr-mb-2w fr-p-0">Trier les modifications</legend>
  <div
    class="flex flex-row flex-wrap items-center gap-2"
    role="group"
    aria-label="Trier les modifications"
  >
