<script lang="ts">
  ...
  import { mailtoJeNetrouvePasUneEspèce } from "@pitchou/common/constantes.ts";
  ...
  async function supprimerEspèceImpactéeDepuisClassification(espèce: EspèceProtégée) {
    ...
    if (indexDansListe >= 0) {
      ...
      if (espècesImpactéesPourPréremplir.length === 0) {
        ...
        // Trouver la référence du bouton correspondant à cette espèce
        // On parcourt les références et on vérifie si l'espèce à cet index correspond
        if (espèceÀFocus) {
          ...
          if (boutonÀFocus) {
            ...
            // Fallback : focus sur le champ texte si pas de bouton disponible
            champTexte?.focus();
          }
        } else {
          // Fallback : focus sur le champ texte
          champTexte?.focus();
        }
      }
    }
...
<div class="fr-modal__header">
  <button
    aria-controls={idModalePréremplirDepuisTexte}
    title="Fermer"
    type="button"
  ...
  <DéplierReplier>
    ...
    {#snippet content()}
      <p class="fr-text--sm">
        ...
        mail à
        <a target="_blank" href={mailtoJeNetrouvePasUneEspèce}>pitchou@beta.gouv.fr</a> en indiquant
...
<div class="fr-modal__footer">
  <button type="button" class="fr-btn fr-btn--secondary fr-ml-auto" onclick={onClickpréciserImpact}
  ...
  <button
    ...
    onclick={préremplirAvecCesEspècesImpacts}
    >{`Ajouter ${espècesImpactéesPourPréremplir.length} ${espècesImpactéesPourPréremplir.length >= 2 ? "espèces" : "espèce"}`}</button
  >
</div>
