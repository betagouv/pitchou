...
function decision(
  overrides: Partial<DécisionAdministrativePourTransfer> = {},
): DécisionAdministrativePourTransfer {
  return reactive({
    dossier: "dossier-test",
    ...overrides,
  } as unknown as DécisionAdministrativePourTransfer);
}
...
test("appelle onValider avec le fichier encodé en base64 quand tout est valide", async () => {
  ...
  const decisionTransmise = onValider.mock.calls[0][0] as DécisionAdministrativePourTransfer;
