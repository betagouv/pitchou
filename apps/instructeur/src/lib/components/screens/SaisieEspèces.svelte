<script lang="ts">
  ...
  async function créerOdsBlob() {
    const odsArrayBuffer = await descriptionMenacesEspècesToOdsArrayBuffer(
      espècesImpactéesParClassification,
    );
  ...
  async function onClickPréRemplirAvecDocumentOds() {
    try {
      ...
      const descriptionMenacesEspèces = await fichierEspècesOds
        .arrayBuffer()
        .then(importDescriptionMenacesEspècesFromOds);

      if (Object.keys(descriptionMenacesEspèces).length >= 1) {
        espècesImpactées =
          impactsParClassificationVerListeEspècesImpactées(descriptionMenacesEspèces);
      ...
      messageErreurPréRemplirAvecDocumentOds =
  ...
  <article>
    ...
    <div class="fr-grid-row">
      <div class="fr-col">
              ...
              <div class="fr-col-12 fr-col-md-8 fr-col-lg-6">
                <div class="fr-modal__body">
                  ...
                  <div class="fr-modal__content">
                    <h2
                      id="modale-préremplir-depuis-import-title-avec-liste"
        ...
        <ModalePréremplirDepuisTexte
          bind:référencesEspèces
          {espècesProtégéesParClassification}
      ...
      {:else}
        ...
        {:then { identifiantPitchouVersActivitéEtImpactsQuantifiés }}
          <EspècesProtégéesGroupéesParImpact
            espècesImpactées={espècesImpactéesParClassification}
    ...
    {:else}
      <FormulaireSaisieEspèce
        bind:espècesImpactées
        bind:référencesEspèces
        espècesProtégées={[
          ...espècesProtégéesParClassification["oiseau"],
          ...espècesProtégéesParClassification["faune non-oiseau"],
          ...espècesProtégéesParClassification["flore"],
        ]}
        {activitesParClassificationEtreVivant}
        {méthodesParClassificationEtreVivant}
        {transportsParClassificationEtreVivant}
