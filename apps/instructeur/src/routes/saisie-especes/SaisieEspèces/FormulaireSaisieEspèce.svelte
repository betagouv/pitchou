<script lang="ts">
  ...
  import TuileSaisieEspèce from "./TuileSaisieEspèce.svelte";
  ...
  async function supprimerEspèce(indexEspèceÀSupprimer: number) {
    ...
    if (espècesImpactées.length === 0) {
      ...
      let indexEspèceÀFocus =
        indexEspèceÀSupprimer === espècesImpactées.length
  ...
  {#each espècesImpactées as espècesImpactée, indexEspècesImpactée (espècesImpactée)}
    <TuileSaisieEspèce
      ...
      onSuprimerEspèce={async () => {
        await supprimerEspèce(indexEspècesImpactée);
      }}
      onDupliquerEspèce={async () => {
        await duppliquerEspèce(indexEspècesImpactée);
      }}
      {espècesProtégées}
      {activitesParClassificationEtreVivant}
      {méthodesParClassificationEtreVivant}
