<script lang="ts">
  ...
  let { dossierId, décisionAdministrative }: Props = $props();
  ...
  let prescriptions: Partial<FrontEndPrescription>[] = $state(
    décisionAdministrative.prescriptions ? [...décisionAdministrative.prescriptions] : [],
  );
  ...
  async function onFileInput(e: Event & { currentTarget: HTMLElement & HTMLInputElement }) {
    ...
    const nouvellesPrescriptions = await créerPrescriptionContrôlesÀPartirDeFichier(
      importPrescriptionFileAB,
      décisionAdministrative,
    );

    prescriptions = nouvellesPrescriptions;
