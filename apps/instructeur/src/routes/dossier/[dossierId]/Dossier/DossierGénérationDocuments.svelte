<script lang="ts">
  ...
  import { getBalisesGénérationDocument } from "./générerDocument.ts";
  ...
  async function generateDoc(e: SubmitEvent) {
    ...
    const { identifiantPitchouVersActivitéEtImpactsQuantifiés } =
      await chargerActivitésMéthodesMoyensDePoursuite();
    ...
    if (!espèces_impacts) {
      // @ts-ignore
      erreurGénérationDocument = new Error(
    ...
    const balises = getBalisesGénérationDocument(
      dossier,
      espèces_impacts,
