...
import { supprimerSuivisDevenusInaccessibles } from "@pitchou/server/database/groupe_instructeurs.ts";
...
test("supprimerSuivisDevenusInaccessibles supprime le suivi quand la personne perd l'accès au dossier", async () => {
  ...
  await supprimerSuivisDevenusInaccessibles(DEFAULT_NUMERO_DEMARCHE, db);

  expect(await countSuivi(personneId, dossier.id)).toBe(0);
});

test("supprimerSuivisDevenusInaccessibles conserve le suivi quand la personne garde l'accès au dossier", async () => {
