...
test.describe("Page Mes dossiers", () => {
  test(`Je peux voir tous les dossiers que je suis, et les dossiers selon le tri suivant :
            ...
            - Puis par date date_dépôt, de la plus récente à la plus ancienne.`, async ({
    page,
  }) => {
  ...
  test(`Les dossiers avec une notification non lue ont un tag Nouveauté.`, async ({ page }) => {
    const dossiersAvecNotificationNonVue = await page
  ...
  test(`Quand j'appuie sur Nouveauté, je ne vois que les dossiers qui possèdent une notification non vue.`, async ({
    page,
  }) => {
  ...
  test(`Quand je consulte un dossier et que je reviens sur la page Mes dossiers, le tag Nouveauté disparaît. `, async ({
    page,
  }) => {
    ...
    const premierDossier = page.getByTestId("carte-dossier").filter({
    ...
    await expect(page.getByRole("heading", { level: 1 })).toContainText(
      "Dossier n°2 : Recherche scientifique sur les chats",
    );
