...
export default async function (cheminRelatifDonnéesSQL) {
  ...
  await exec(
    `docker exec pitchou-test_db-1 psql --username=dev especes_pro_3731 -f /docker-entrypoint-initdb.d/01-schema.sql`,
  );
  await exec(
    `docker exec pitchou-test_db-1 psql --username=dev especes_pro_3731 -f /docker-entrypoint-initdb.d/02-knex.sql`,
  );

  // Exécuter les migrations spécifiques à ce test
  await exec(
    `docker exec pitchou-test_db-1 psql --username=dev especes_pro_3731 -f /tests/${cheminRelatifDonnéesSQL}`,
  );
}
