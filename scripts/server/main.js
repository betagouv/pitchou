...
import {
  ...
  getDossierComplet,
  getDossierMessages,
  getDossiersRésumésByCap,
  getÉvènementsPhaseDossiers,
  updateDossier,
} from "./database/dossier.js";
import {
  créerPersonneOuMettreÀJourCodeAccès,
  getPersonneByDossierCap,
  getPersonneByEmail,
} from "./database/personne.js";
...
} from "./database/relation_suivi.js";
...
fastify.post("/lien-preremplissage", async function (request) {
  ...
  const donnéesSupplémentairesDossiers =
    donnéesPreRemplissage["NE PAS MODIFIER - Données techniques associées à votre dossier"];
  ...
  if (donnéesSupplémentairesDossiers) {
    donnéesPreRemplissage["NE PAS MODIFIER - Données techniques associées à votre dossier"] =
      await chiffrerDonnéesSupplémentairesDossiers(donnéesSupplémentairesDossiers);
  }

  return (
    demanderLienPréremplissage(donnéesPreRemplissage, schema88444)
      // @ts-ignore
...
async function téléchargementFichierRouteHandler(request, reply) {
  ...
  if (!fichier) {
    ...
    const nomDuFichierAscii = fichier.nom
      .normalize("NFD")
  ...
  async function (request, reply) {
    ...
    if (!dossiersAccessibles.has(décisionData.dossier)) {
      reply
        .code(400)
        .send(
          `La capability ${cap} ne permet pas d'avoir accès au dossier ${décisionData.dossier}`,
        );
...
fastify.delete(
  "/decision-administrative/:decisionAdministrativeId",
  async function (request, reply) {
    //@ts-ignore
...
fastify.post("/prescription", function (request, reply) {
  ...
  return ret
    .then((/** @type {Prescription['id'] | undefined} */ prescriptionId) => {
...
fastify.post("/contrôle", function (request, reply) {
  ...
  return ret
    .then((/** @type {Contrôle['id'] | undefined} */ contrôleId) => {
...
fastify.post(
  ...
  async function (req) {
    ...
    const [blobFichierSaisineContenu, blobFichierAvisContenu] = await Promise.all([
      "blobFichierSaisine" in body ? await body.blobFichierSaisine.toBuffer() : Promise.resolve([]),
    ...
    if ("blobFichierSaisine" in body) {
      ...
      fichierSaisine = {
        nom: blobFichierSaisine.filename,
        media_type: blobFichierSaisine.mimetype,
        contenu: blobFichierSaisineContenu,
      };
    }
    if ("blobFichierAvis" in body) {
      ...
      fichierAvis = {
        nom: blobFichierAvis.filename,
        media_type: blobFichierAvis.mimetype,
        contenu: blobFichierAvisContenu,
      };
    }
  ...
  if (direction === "suivre") {
    ...
    if (direction !== "laisser") {
      reply
        .code(500)
        .send(
          `Erreur lors du changement de suivi du dossier. Direction ${direction} non reconnue.`,
        );
      return;
    }
  ...
  return ret
    ...
    .catch((/** @type {any} */ err) => {
      reply
        .code(500)
        .send(
          `Erreur lors du changement de suivi du dossier ${dossierId} par ${personneEmail}. ${err}`,
        );
    });
});
...
async function shutdown(signal) {
  ...
  await Promise.all([closeDatabaseConnection(), fastify.close()]);
  process.exit();
}
