...
function isOngletValide(onglet) {
  return ["instruction", "projet", "echanges", "avis", "controles", "generation-document"].includes(
    onglet,
  );
}
...
export default async ({ params: { dossierId } }) => {
  ...
  function mapStateToProps(state) {
    ...
    let personnesQuiSuiventDossier = relationSuivis
    ...
    let dossiersSuiviParInstructeurActuel =
      relationSuivis && relationSuivis.get(mapStateToSqueletteProps(state)?.email ?? "");
    let dossierActuelSuiviParInstructeurActuel =
      dossiersSuiviParInstructeurActuel && dossiersSuiviParInstructeurActuel.has(dossier.id);

    // Récupérer l'info de l'onglet sélectionné
    const hash = location.hash;
    const onglet = hash.slice("#".length);
    /** @type {Onglet} */
    const ongletActifInitial = onglet && isOngletValide(onglet) ? onglet : "instruction";
