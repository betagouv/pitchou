...
if (typeof _trisFiltresSélectionnés === "string") {
  console.warn(
    `string du localStorage non comprise en tant que filtre/tri`,
    _trisFiltresSélectionnés,
  );
...
function mapStateToPropsSuiviInstruction(state) {
  ...
  const activitésPrincipalesChamp = schemaChamps.find(
    (champDescriptor) => champDescriptor.label === "Activité principale",
  );
...
export default async () => {
  ...
  if (store.state.capabilities.listerDossiers) {
    await chargerDossiers().catch((err) => {
