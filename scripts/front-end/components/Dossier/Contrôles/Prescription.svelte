<script>
  ...
  async function créerContrôle() {
    if (contrôleEnCours) {
      ...
      if (
        contrôleEnCours.résultat === "Conforme" && // qui est conforme
...
<section class="prescription-consultée">
  <DéplierReplier>
    {#snippet summary()}
      ...
      <h6>
        <TagRésultatContrôle résultatContrôle={dernierContrôle?.résultat || NON_RENSEIGNÉ}
        ></TagRésultatContrôle>
        {#if description}
          {description}
    ...
    {#snippet content()}
      <section>
        ...
        <section class="contrôles">
          <h6>
            {#if contrôles.size === 1}1 contrôle{:else}{contrôles.size} contrôles
            {/if}
          </h6>
          ...
          {#each contrôlesTriés as contrôle}
            {#if contrôle === contrôleEnModification}
              ...
              <FormulaireContrôle
                contrôle={contrôleEnModification}
                onValider={validerModificationsContrôle}
              >
                {#snippet boutonAnnuler()}
                  <button
                    type="button"
            ...
            {:else}
              <section class="contrôle">
                <h6>
                  Contrôle du <time datetime={contrôle.date_contrôle?.toISOString()}
                    >{formatDateAbsolue(contrôle.date_contrôle)}</time
                  >
                  <TagRésultatContrôle résultatContrôle={contrôle.résultat || NON_RENSEIGNÉ}
                  ></TagRésultatContrôle>
                  <button
