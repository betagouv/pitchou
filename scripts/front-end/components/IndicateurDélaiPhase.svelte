<script>
  ...
  let weekDiff = $derived(
    differenceInWeeks(new Date(), addMonths(débutPhaseActuelle.dateDébut, monthDiff)) + 1,
  );
  // $: console.log('weekDiff', weekDiff)

  let quantité = $derived(monthDiff + weekDiff / 4);
  let alt = $derived(
    `depuis ${format(débutPhaseActuelle.dateDébut, "yyyy-MM-dd")} - ~${monthDiff} mois`,
  );
</script>
...
{#if débutPhaseActuelle.phase === "Instruction"}
  ...
  ></IndicateurDélai>
{:else}
  <IndicateurDélai {quantité} style="info" {alt}></IndicateurDélai>
{/if}
